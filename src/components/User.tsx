...
const renderFirstValue = (value: string, index: number, letter: string) => {
  let result = "";
  if (!index) {
    result = value[0];
  } else {
...
const Users = ({ name, phoneNumber, index, letter, tags }: userProps) => {
  const theme = useTheme();
  console.log("tags", tags);
  return (
    <Box>
      <Typography variant="body1" sx={{ fontWeight: "600" }}>
        {renderFirstValue(name, index, letter) &&
          renderFirstValue(name, index, letter)}
      </Typography>
      <Box display="flex" alignItems="center" justifyContent="space-between">
        ...
        <Box display="flex" alignItems="center">
          ...
          <AddCircleIcon
            sx={{ color: theme.palette.primary.main, marginLeft: "10px" }}
          />
        </Box>
      </Box>
    </Box>
